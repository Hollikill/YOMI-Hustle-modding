...
func save_player_style(style, player_name):
	make_custom_folder()
	var file = File.new()
	var filename_ = "user://custom/" + player_name + "_" + style.style_name + ".style"
...
#	make_custom_folder()
#	var file = File.new()
#	var filename_ = "user://custom/plaintext"+ style.style_name + ".txt"
#	file.open(filename_, File.WRITE)
#	file.store_string(iterateDict(style))
#	file.close()
#	return filename_
#
#func iterateDict(dict, prefix=""):
#	var output = ""
#	for item in dict:
#		output += prefix + "" + item + "\n"
#		if dict.get(item) != null:
#			var item_value = dict.get(item)
#			if item_value is Dictionary:
#				output += iterateDict(item_value, prefix+"--  ")
#			else:
